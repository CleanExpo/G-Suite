...
async function scanGitHub(query: string, limit: number): Promise<AgentDossier[]> {
...
async function analyzeGitHubRepo(repo: GitHubRepo): Promise<AgentDossier | null> {
  ...
  if (
    !agentKeywords.some((kw) => description.includes(kw) || repo.name.toLowerCase().includes(kw))
  ) {
    return null;
  }
  ...
  if (description.includes('multi-agent') || description.includes('multiagent'))
    capabilities.push('multi_agent');
...
async function scanHuggingFace(query: string, limit: number): Promise<AgentDossier[]> {
...
async function scanWithAI(source: string, query: string, limit: number): Promise<AgentDossier[]> {
