...
export async function deepResearch(
  ...
  const model = genAI.getGenerativeModel({
    ...
    systemInstruction:
      'You are a research analyst. Provide comprehensive, well-sourced analysis based on your internal knowledge.',
  });
...
export async function veo31Generate(
  ...
  const hasGoogleCloud =
    process.env.GOOGLE_CLOUD_PROJECT && process.env.GOOGLE_CLOUD_PROJECT !== 'your-project-id-here';
  ...
  try {
    ...
    const waitForCompletion =
      options.waitForCompletion !== undefined ? options.waitForCompletion : true; // Default: wait for completion
      ...
      result = await veoClient.generateVideoAndWait(generationOptions, {
        ...
        onProgress: (attempt, maxAttempts) => {
          if (attempt % 6 === 0) {
            // Log every minute
            console.log(
              `[veo31Generate] Polling... ${attempt}/${maxAttempts} (${Math.round((attempt / maxAttempts) * 100)}%)`,
            );
          }
        },
      });
...
export async function documentAIExtract(
  ...
  try {
    ...
    return {
      ...
      tables:
        extractType === 'table' || extractType === 'all'
          ? [
              {
