  ...
  try {
    ...
    if (loadTime < 2000) {
      ...
      result.recommendations.push(
        `Optimize LCP: The site load time is ${loadTime}ms. Target < 1.5s.`,
      );
    }
    ...
    const analysis = await page.evaluate(() => {
      ...
      const hasPrivacy = !!Array.from(document.querySelectorAll('a')).find((a) =>
        a.innerText.toLowerCase().includes('privacy'),
      );
      const missingAlts = Array.from(document.querySelectorAll('img')).filter(
        (img) => !img.alt,
      ).length;
    ...
    if (analysis.hasSchema) {
      ...
      result.recommendations.push(
        'Add JSON-LD: Implement SoftwareApplication schema to improve SERP snippets.',
      );
    }
    ...
    if (analysis.hasPrivacy) {
      ...
      result.recommendations.push(
        'Add Privacy Policy: Visible compliance links are critical for E-E-A-T trust.',
      );
    }
    ...
    if (analysis.missingAlts === 0) {
      ...
      result.recommendations.push(
        `Fix Accessibility: Found ${analysis.missingAlts} images missing alt text.`,
      );
    }
    ...
    if (analysis.h1Count === 1) {
      ...
      result.recommendations.push(
        `Fix SEO: Found ${analysis.h1Count} H1 tags. Exactly one H1 per page is required.`,
      );
    }
