...
export class DocumentProcessor {
    ...
    async process(request: DocumentProcessingRequest): Promise<DocumentProcessingResult> {
        ...
        try {
            ...
            const invoiceData = classification.type === 'invoice' ? this.extractInvoiceData(processedDoc) : undefined;
            const receiptData = classification.type === 'receipt' ? this.extractReceiptData(processedDoc) : undefined;
            const contractData = classification.type === 'contract' ? this.extractContractData(processedDoc) : undefined;
    ...
    private classifyDocument(doc: ProcessedDocument, fileName?: string): DocumentClassification {
        ...
        if (fileName?.toLowerCase().includes('resume') || text.includes('experience') && text.includes('education')) {
    ...
    private generateInsights(doc: ProcessedDocument): DocumentInsights {
        ...
        const moneyEntities = doc.entities.filter(e => e.type.includes('amount') || e.type.includes('money'));
        const dateEntities = doc.entities.filter(e => e.type.includes('date'));
        const orgEntities = doc.entities.filter(e => e.type.includes('org') || e.type.includes('supplier') || e.type.includes('company'));
        const personEntities = doc.entities.filter(e => e.type.includes('person') || e.type.includes('name'));
        const locEntities = doc.entities.filter(e => e.type.includes('address') || e.type.includes('location'));

        return {
            keyAmounts: moneyEntities.map(e => parseFloat(e.normalizedValue || '0')).filter(n => !isNaN(n)),
