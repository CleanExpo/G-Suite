...
export class RankingPredictor {
   ...
   */
  predictRank(factors: RankingFactors): {
    probability: number;
    ...
    const baseScore =
      factors.authorityScore * 0.4 + factors.reviewScore * 0.3 + factors.categoryRelevance * 30;
  ...
  generateRankingGrid(
    ...
    radiusKm: number,
    stepKm: number,
    baseFactors: Omit<RankingFactors, 'distanceKm'>,
  ) {
    const grid = [];
