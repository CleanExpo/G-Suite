...
export class LocalCompetitorAnalyzer {
     ...
     */
    private async findLocalCompetitors(
    ...
    compareGBPProfiles(
        ...
        const avgReviews = competitorReviewCounts.reduce((a, b) => a + b, 0) / competitorReviewCounts.length;
        ...
        if (yourProfile.reviewCount < avgReviews * 0.7) {
            insights.push(`You have ${Math.round(avgReviews - yourProfile.reviewCount)} fewer reviews than average`);
        ...
        const avgPhotos = competitorPhotoCounts.reduce((a, b) => a + b, 0) / competitorPhotoCounts.length;
        ...
        const avgCompleteness = competitorCompleteness.reduce((a, b) => a + b, 0) / competitorCompleteness.length;
    ...
    analyzeCitationGap(
        ...
        const avgCitations = competitorCitations.reduce((a, b) => a + b, 0) / competitorCitations.length;
    ...
    compareReviews(
        ...
        const avgRating = competitorReviews.reduce((a, b) => a + b.rating, 0) / competitorReviews.length;
        const avgReplyRate = competitorReviews.reduce((a, b) => a + b.replyRate, 0) / competitorReviews.length;

        // Review count analysis
        if (yourReviews.count < avgCount * 0.5) {
            insights.push(`You have ${Math.round(avgCount - yourReviews.count)} fewer reviews than average`);
            recommendations.push('Implement a review generation campaign - ask satisfied customers for reviews');
        ...
        if (yourReviews.replyRate < avgReplyRate - 10) {
            ...
            recommendations.push('Respond to all reviews, especially negative ones, to show customer engagement');
        ...
        if (yourVelocity < avgVelocity * 0.5) {
            recommendations.push('Increase review frequency - competitors are getting more reviews per month');
        }
    ...
    identifyKeywordGaps(
        ...
        missingKeywords.sort((a, b) =>
            allCompetitorKeywords[b] - allCompetitorKeywords[a]
        );
        ...
        if (missingKeywords.length > 0) {
            opportunities.push(
                `${missingKeywords.length} keywords used by competitors but not by you`
            );
            ...
            if (top3.length > 0) {
                opportunities.push(
                    `Top missing keywords: ${top3.join(', ')}`
                );
            }
        } else {
    ...
    private identifyGaps(
        ...
        const avgCitations = competitors.reduce((sum, c) => sum + c.citationCount, 0) / competitors.length;
        ...
        if (yourProfile.photoCount < 10) {
            const avgPhotos = competitors.reduce((sum, c) =>
                sum + (c.strengths.includes('photos') ? 20 : 5), 0
            ) / competitors.length;
    ...
    private generateSummary(
        ...
        const competitorScores = competitors.map(c =>
            c.rating * 20 +
            Math.min(c.reviewCount / 5, 20) +
            Math.min(c.citationCount / 2, 10)
        );
