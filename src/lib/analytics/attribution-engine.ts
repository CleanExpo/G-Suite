...
export class AttributionEngine {
    ...
    calculateAttribution(
        ...
        for (const path of paths) {
            ...
            for (const [key, credit] of Object.entries(credits)) {
                ...
                if (existing) {
                    ...
                    const touchPoint = path.touchPoints.find(
                        t => `${t.source}/${t.medium}` === key
                    ) || path.touchPoints[0];
        ...
        const totalRevenue = Array.from(results.values())
            .reduce((sum, r) => sum + r.attributedRevenue, 0);

        for (const result of results.values()) {
            result.contributionPercentage = totalRevenue > 0
                ? (result.attributedRevenue / totalRevenue) * 100
                : 0;
        }
