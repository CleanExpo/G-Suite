...
export interface ProcessDocumentOptions {
    ...
    mimeType: 'application/pdf' | 'image/png' | 'image/jpeg' | 'image/tiff' | 'image/gif' | 'image/bmp';
}
...
export class DocumentAIClient {
    ...
    constructor(config?: Partial<DocumentAIConfig>) {
        ...
        if (!projectId || !processorId) {
            console.warn('[DocumentAIClient] Missing configuration. Document processing will be disabled.');
    ...
    async processDocument(options: ProcessDocumentOptions): Promise<ProcessedDocument> {
        if (!this.isConfigured || !this.client) {
            throw new Error('Document AI client is not configured. Please set GOOGLE_CLOUD_PROJECT and DOCUMENT_AI_PROCESSOR_ID environment variables.');
        }
     ...
     */
    async processFile(filePath: string, mimeType: ProcessDocumentOptions['mimeType']): Promise<ProcessedDocument> {
    ...
    private extractTableData(table: any, text: string): TableData {
        ...
        const columnCount = headerRows.length > 0 ? headerRows[0].length : (bodyRows.length > 0 ? bodyRows[0].length : 0);
