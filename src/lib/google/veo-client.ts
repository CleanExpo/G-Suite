...
export class VeoClient {
    ...
    async getOperationStatus(operationName: string): Promise<VeoOperationStatus> {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}));
            throw new Error(`Failed to get operation status (${response.status}): ${errorData.error?.message || response.statusText}`);
        }
    ...
    async generateVideoAndWait(
        ...
        const finalResult = await this.pollOperationStatus(
