...
export class OnPageAnalyzer {
  ...
  private analyzeTitle(doc: Document, keyword: string): TitleAnalysis {
    ...
    if (length < 30) {
      ...
      recommendations.push(
        'Shorten title to 50-60 characters to avoid truncation in search results',
      );
      score -= 15;
    }
    ...
    const duplicates = words.filter(
      (word, index) => words.indexOf(word) !== index && word.length > 3,
    );
  ...
  private analyzeMetaDescription(doc: Document, keyword: string): MetaDescriptionAnalysis {
    ...
    const hasCallToAction =
      /\b(learn|discover|find|get|try|start|explore|see|read|download|subscribe|buy|shop|order|book)\b/i.test(
        content,
      );
    ...
    for (let level = 2; level <= 6; level++) {
      if (headings[`h${level}`].length > 0) {
        if (level - previousLevel > 1) {
          missingLevels.push(
            ...Array.from({ length: level - previousLevel - 1 }, (_, i) => previousLevel + i + 1),
          );
        }
        previousLevel = level;
      }
  ...
  private analyzeImages(doc: Document, baseUrl: string): ImageAnalysis {
    ...
    const withTitle = images.filter(
      (img) => img.title !== undefined && img.title.length > 0,
    ).length;
    ...
    if (optimized < total * 0.5) {
      issues.push(
        `Only ${optimized}/${total} images optimized (use WebP/AVIF or specify dimensions)`,
      );
      score -= 20;
    }
    ...
    if (missingDimensions > 0) {
      issues.push(
        `${missingDimensions} image${missingDimensions > 1 ? 's' : ''} missing width/height attributes (causes layout shift)`,
      );
      score -= 15;
    }
  ...
  private analyzeLinks(doc: Document, baseUrl: string): LinkAnalysis {
    ...
    const nofollow = [...internalLinks, ...externalLinks].filter((link) =>
    ...
    if (internal === 0) {
      ...
      issues.push(
        `Only ${internal} internal link${internal > 1 ? 's' : ''} - add more for better site structure`,
      );
      score -= 10;
    }

    // Check for links without anchor text
    const emptyText = [...internalLinks, ...externalLinks].filter(
    ...
    if (genericText > 0) {
      issues.push(
        `${genericText} link${genericText > 1 ? 's' : ''} with generic anchor text ("click here", etc.)`,
      );
      score -= 10;
    }
    ...
    if (externalWithoutRel > 0) {
      issues.push(
        `${externalWithoutRel} external link${externalWithoutRel > 1 ? 's' : ''} without rel="noopener" or rel="nofollow"`,
      );
      score -= 10;
    }
