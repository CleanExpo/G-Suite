  ...
  private generateRecommendations(analysis: {
    ...
    if (analysis.onPageSEO.headings.issues.length > 0) {
      recommendations.push({
        ...
        effort: 'medium',
        implementation:
          'Ensure proper heading hierarchy (one H1, multiple H2s for subheadings) and include target keywords.',
      });
    }
    ...
    if (analysis.onPageSEO.images.issues.length > 0) {
      recommendations.push({
        ...
        effort: 'medium',
        implementation:
          'Add descriptive alt text to all images and specify width/height attributes to prevent layout shift.',
      });
    }
    ...
    if (analysis.technicalSEO.performance.issues.length > 0) {
      recommendations.push({
        ...
        effort: 'high',
        implementation:
          'Optimize images, minimize JavaScript, enable compression, and use browser caching.',
      });
    }
    ...
    if (analysis.technicalSEO.mobile.issues.length > 0) {
      recommendations.push({
        ...
        effort: 'medium',
        implementation:
          'Add viewport meta tag, increase touch target sizes, and use legible font sizes.',
      });
    }
    ...
    if (!analysis.technicalSEO.structured.hasStructuredData) {
      recommendations.push({
        ...
        description:
          'No structured data found. Adding schema markup improves rich snippets in search results.',
        impact: 15,
        effort: 'medium',
        implementation:
          'Add JSON-LD structured data for Organization, WebSite, and WebPage schemas.',
      });
    }
    ...
    if (analysis.contentAnalysis.contentQuality.issues.length > 0) {
      recommendations.push({
        ...
        effort: 'high',
        implementation:
          'Expand content length, improve content-to-code ratio, and remove duplicate paragraphs.',
      });
    }
    ...
    if (analysis.onPageSEO.openGraph.missingTags.length > 0) {
      recommendations.push({
        ...
        effort: 'low',
        implementation:
          'Add og:title, og:description, og:image, and og:url for better social media sharing.',
      });
    }
  ...
  private generateIssues(analysis: {
    ...
    if (!analysis.technicalSEO.security.hasHTTPS) {
      issues.push({
        ...
        description:
          'Your site is not using a secure HTTPS connection. This is a ranking factor and affects user trust.',
    ...
    if (!analysis.technicalSEO.crawlability.indexable) {
      issues.push({
        ...
        description:
          'This page has a noindex meta tag, preventing it from appearing in search results.',
    ...
    if (analysis.contentAnalysis.wordCount < 300) {
      issues.push({
        ...
        fixInstructions:
          'Expand content to at least 600-1000 words with valuable, in-depth information.',
