...
export class InstagramAdapter extends BaseSocialMediaAdapter {
  ...
  private async publishSingleImage(
    ...
    if (!containerResponse.ok) {
      ...
      throw new Error(
        `Failed to create media container: ${error.error?.message || containerResponse.statusText}`,
      );
    }
    ...
    if (!publishResponse.ok) {
      ...
      throw new Error(
        `Failed to publish media: ${error.error?.message || publishResponse.statusText}`,
      );
    }
  ...
  private async publishCarousel(
    ...
    const carouselResponse = await fetch(`${this.baseUrl}/${accountId}/media`, {
      method: 'POST',
      body: carouselParams,
    });
  ...
  private async publishStory(
    ...
    throw new Error(
      'Instagram Stories publishing requires Instagram Graph API review and special permissions',
    );
  }
