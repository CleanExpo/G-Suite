...
export class PrismaMissionAdapter {
     ...
     */
    static async createMission(userId: string, cost: number = 0, status: string = 'PENDING'): Promise<string | null> {
        try {
            ...
            console.warn('⚠️ Mission persistence failed (Create):', error instanceof Error ? error.message : error);
            return null;
        }
    }
    ...
    static async updatePlan(missionId: string, plan: AgentPlan): Promise<void> {
        ...
        try {
            ...
            console.warn('⚠️ Mission persistence failed (UpdatePlan):', error instanceof Error ? error.message : error);
        }
    }
    ...
    static async completeMission(
        ...
        try {
            ...
            console.warn('⚠️ Mission persistence failed (Complete):', error instanceof Error ? error.message : error);
        }
    }
}
