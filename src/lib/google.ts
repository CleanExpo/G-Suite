...
export async function getGoogleAuth(userId?: string) {
    ...
    if (profile?.googleApiKey) {
      try {
        ...
        if (decryptedKey.includes('{')) {
          ...
          console.warn(
            `User ${userId} provided a plain API key. G-Pilot missions require a Service Account JSON for Slides/Drive.`,
          );
        }
      } catch (err) {
  ...
  if (!clientEmail || !privateKey) {
    throw new Error(
      "Missing Google Service Account credentials. Initialize 'The Vault' in onboarding.",
    );
  }
