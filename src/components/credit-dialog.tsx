    ...
    const packages = [
        {
            ...
            btnColor: "bg-gray-100 dark:bg-white/10 text-gray-900 dark:text-white hover:bg-gray-200 dark:hover:bg-white/20",
                    ...
                    >
                        <div className="p-10 md:p-16">
                            <div className="flex justify-between items-start mb-16">
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Ledger Replenishment</span>
                                    </div>
                                    ...
                                    <p className="text-gray-600 dark:text-gray-400 font-medium text-lg">Select a mission-driven fuel package for your autonomous fleet.</p>
                                </div>
                                <button onClick={onClose} className="p-4 bg-gray-100 dark:bg-white/5 rounded-2xl hover:scale-110 transition-transform"><X className="w-6 h-6 text-gray-400" /></button>
                            </div>
                                ...
                                {packages.map((pkg) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-4 mb-8">
                                            <div className={`p-3 rounded-2xl border ${pkg.popular ? 'bg-white/20 border-white/20' : 'bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-800'}`}>
                                                {pkg.icon}
                                            </div>
                                        ...
                                        <div className="mb-10">
                                            ...
                                            <div className={`text-[10px] font-black uppercase tracking-widest mt-2 ${pkg.popular ? 'text-white/60' : 'text-gray-400'}`}>
                                        ...
                                        <ul className="space-y-4 mb-12 flex-grow">
                                            {pkg.features.map((f, i) => (
                                                <li key={i} className="flex items-start gap-3 text-sm font-bold uppercase tracking-tight">
                                                    <Check className={`w-5 h-5 shrink-0 ${pkg.popular ? 'text-blue-200' : 'text-blue-600'}`} />
                                                    <span className={pkg.popular ? 'text-white/90' : 'text-gray-600 dark:text-gray-400 text-xs'}>{f}</span>
                                                </li>
                                            ))}
                                        </ul>
                            ...
                            <div className="mt-16 flex items-center justify-center gap-6 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                                <div className="flex items-center gap-2"><Shield className="w-4 h-4 text-emerald-500" /> Stripe Secure</div>
                                <div className="flex items-center gap-2"><Check className="w-4 h-4 text-blue-600" /> Identity Verified</div>
                            </div>
                        </div>
                    </motion.div>
                </div>
