...
export default function MissionModal({ isOpen, onClose, onSuccess, initialInput }: MissionModalProps) {
                        ...
                        <div className="relative flex items-center justify-between p-10 border-b border-gray-50 dark:border-white/5">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mt-1">Orchestrator v4.2 · Secure Linkage</p>
                                </div>
                            </div>
                            <button onClick={onClose} className="p-3 hover:bg-gray-100 dark:hover:bg-white/5 rounded-2xl transition-all"><X className="w-6 h-6 text-gray-400" /></button>
                        </div>
                        ...
                        <div className="p-10 space-y-10">
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-[10px] font-black text-emerald-500 uppercase">Input Encrypted</span>
                                </div>
                                <textarea
                                    value={input}
                            ...
                            {error && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-800 text-rose-600 dark:text-rose-400 text-xs font-bold rounded-2xl flex items-center gap-3">
                                    <AlertCircle className="w-4 h-4" /> {error}
                                </motion.div>
                            )}

                            <div className="flex gap-4">
                                <button onClick={onClose} disabled={isLaunching} className="flex-1 h-16 rounded-2xl font-black text-gray-400 hover:text-gray-900 dark:hover:text-white transition-all uppercase tracking-widest text-xs">Cancel</button>
                                <button
                                ...
                                >
                                    {isLaunching ? <Loader2 className="w-6 h-6 animate-spin" /> : <>Deploy Mission <Send className="w-5 h-5 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /></>}
                                </button>
                            </div>
                        </div>
                    </motion.div>
