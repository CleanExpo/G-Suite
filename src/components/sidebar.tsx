...
const menuItems = [
    ...
    { icon: Zap, label: 'Campaign Architect', href: '/dashboard/campaigns', color: 'text-yellow-500', badge: 'Hot' },
    { icon: Mic2, label: 'Multimodal Live', href: '/dashboard/live', color: 'text-purple-500', badge: 'Beta' },
    { icon: MapPin, label: 'GEO Marketing', href: '/dashboard/geo', color: 'text-emerald-500' },
    { icon: FileText, label: 'Document Intel', href: '/dashboard/documents', color: 'text-amber-500' },
    { icon: Search, label: 'Deep Research', href: '/dashboard/research', color: 'text-indigo-500', badge: 'New' },
    { icon: LayoutDashboard, label: 'Ad-Hoc Marketplace', href: '/dashboard/marketplace', color: 'text-emerald-400', badge: 'Pro' },
    ...
    return (
        ...
        >
                ...
                {menuItems.map((item) => {
                    ...
                    return (
                        <Link key={item.href} href={item.href}>
                            ...
                            `}>
                                <item.icon className={`w-5 h-5 shrink-0 ${isActive ? item.color : 'group-hover/item:text-gray-300'}`} />
                                {!isCollapsed && (
            ...
            <div className="p-4 border-t border-white/5 space-y-4">
                ...
                <div className={`p-4 rounded-2xl bg-gradient-to-br from-blue-600/10 to-purple-600/10 border border-white/5 ${isCollapsed ? 'hidden' : 'block'}`}>
                    <div className="flex items-center gap-2 mb-2">
                        <Zap className="w-4 h-4 text-amber-400" />
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">Mission Status</span>
                    </div>
