...
export async function POST(req: NextRequest): Promise<Response> {
  try {
    ...
    console.log(
      `[POST /api/geo/rankings] Predicting ranking for "${body.keyword}" in ${body.location}`,
    );
...
export async function GET(): Promise<Response> {
  return NextResponse.json({
    ...
    data: {
      ...
      requiredFields: ['keyword', 'location', 'businessName'],
      optionalFields: ['distanceKm', 'authorityScore', 'reviewScore', 'categoryRelevance'],
    },
  });
}
