...
export async function POST(req: NextRequest): Promise<Response> {
  try {
    ...
    try {
      ...
      console.warn(
        '[POST /api/geo/analyze] Clerk authentication not configured, allowing unauthenticated access',
      );
...
export async function GET(req: NextRequest): Promise<Response> {
  return NextResponse.json({
    ...
    data: {
      ...
      requiredFields: ['url', 'businessName', 'address', 'phone', 'targetLocation'],
      optionalFields: ['targetKeywords', 'categories', 'serviceArea'],
    },
  });
}
