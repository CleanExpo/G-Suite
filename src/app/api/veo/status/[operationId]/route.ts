...
export async function GET(
    ...
    try {
        ...
        console.log(`[GET /api/veo/status/${operationId}] Checking operation status for user ${userId}`);
            ...
            return NextResponse.json(
                {
                    ...
                    error: {
                        code: 'NOT_CONFIGURED',
                        message: 'Google Cloud not configured. Set GOOGLE_CLOUD_PROJECT in environment variables.'
                    }
                },
                { status: 503 }
            );
