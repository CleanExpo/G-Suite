...
export default function LivePage() {
    ...
    const config: LiveConfig = {
        ...
        system_instruction: {
            parts: [{ text: 'You are G-Pilot, a helpful AI project manager. You can hear and see the user in real-time. Be concise, professional, and proactive.' }]
        }
    };
            ...
            <main className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden">
                ...
                <div className="bg-slate-900/50 rounded-2xl border border-slate-800 flex flex-col overflow-hidden">
                    <div className="p-4 border-b border-slate-800 bg-slate-900/80">
                        <h2 className="font-bold text-sm tracking-widest uppercase text-slate-400">Activity Log</h2>
                    </div>

                    <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-sm">
                        {transcript.length === 0 ? (
                            <div className="text-slate-600 italic">No activity yet. Start the session to begin...</div>
                        ) : (
                            transcript.map((line, idx) => (
                                <div key={idx} className={`${line.startsWith('AI:') ? 'text-emerald-400' : 'text-blue-400'}`}>
                    ...
                    <div className="p-4 border-t border-slate-800 space-y-4">
                        ...
                        <button
                            ...
                            disabled={!connected}
                            className={`w-full py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 ${isRecording
