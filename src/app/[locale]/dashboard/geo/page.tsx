                    ...
                    {auditResult ? (
                        <div className="space-y-8">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="text-sm text-slate-400 uppercase tracking-widest font-mono">Trust Score</div>
                                    <div className={`text-xs ${auditResult.consistent ? 'text-emerald-500' : 'text-amber-500'}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {auditResult.citations.map((c: any, i: number) => (
                                    <div key={i} className="bg-black/30 p-4 rounded-lg border border-slate-800 flex justify-between items-center">
                                        <div>
                                            <div className="font-bold text-slate-200">{c.source}</div>
                                            <div className="text-xs text-slate-500">{c.foundPhone || 'No mismatches'}</div>
                                        </div>
                                        <div className={`px-2 py-1 rounded text-[10px] uppercase font-bold ${c.status === 'match' ? 'bg-emerald-900/50 text-emerald-400' :
                                                c.status === 'mismatch' ? 'bg-amber-900/50 text-amber-400' :
                            ...
                            <div className="bg-blue-900/10 border border-blue-900/30 p-4 rounded-lg">
                                <h3 className="text-sm font-bold text-blue-400 mb-2 uppercase">Priority Recommendations</h3>
                ...
                <div className="aspect-[21/9] bg-black/40 rounded-xl flex items-center justify-center relative overflow-hidden group">
                    ...
                    <div className="absolute inset-0 opacity-20 grid grid-cols-10 grid-rows-5 gap-1 p-4">
                        {Array.from({ length: 50 }).map((_, i) => (
                            <div key={i} className={`rounded ${i % 7 === 0 ? 'bg-red-500' : i % 3 === 0 ? 'bg-emerald-500' : 'bg-blue-500'}`} />
                        ))}
                    </div>
                    <div className="relative z-10">
                        <span className="text-slate-500 font-mono">Heatmap visualization requires Google Maps API credentials</span>
                        <div className="mt-4 flex justify-center gap-8 text-xs text-slate-400">
                            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded bg-emerald-500" /> Top 3 Probable</div>
                            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded bg-blue-500" /> Top 10 Probable</div>
                            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded bg-red-500" /> Low Visibility</div>
                        </div>
                    </div>
                </div>
            </div>
