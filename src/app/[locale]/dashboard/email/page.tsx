            ...
            <div className="max-w-7xl mx-auto">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        {/* Compose Tab */}
                        {activeTab === 'compose' && (
                            <ComposeForm templates={templates} onSuccess={loadData} />
                        )}
                    </>
                )}
            </div>
    ...
    return (
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-xl p-6">
            <div className="flex items-start justify-between mb-4">
                ...
                <span className={`${statusColors[campaign.status]} px-3 py-1 rounded-full text-xs font-medium text-white capitalize`}>
                    {campaign.status}
                </span>
            </div>
...
function StatBox({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
    return (
        <div className="text-center">
            <div className={`text-2xl font-bold ${highlight ? 'text-blue-400' : 'text-white'}`}>{value}</div>
...
function ComposeForm({ templates, onSuccess }: { templates: EmailTemplate[]; onSuccess: () => void }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-xl p-6">
            ...
            <div className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-400 mb-2">Recipients (one email per line)</label>
                    <textarea
                        value={recipients}
