    ...
    const plans = [
        {
            ...
            features: ["5 Sandbox Agents", "Standard Intel Scraping", "Google Slides Export", "Community Uplink"],
            accent: "blue"
        },
        {
            name: "Admiral",
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-32">
                    <header className="space-y-8">
                        ...
                        <p className="text-2xl text-gray-600 dark:text-gray-400 leading-relaxed font-medium">
                            Transparent, mission-driven cost modeling. No hidden tokens.
                            No enterprise bloat. You pay for the compute intelligence you deploy.
                        </p>
                    </header>
                    <motion.div
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12 items-stretch">
                    {plans.map((plan, i) => (
                        ...
                        >
                            ...
                            <div className="mb-12">
                                <h3 className="text-xs uppercase tracking-[0.3em] font-black text-gray-400 mb-4">{plan.name}</h3>
                                <div className="text-7xl font-black text-gray-900 dark:text-white tracking-tighter mb-6 flex items-baseline">
                                    {plan.price}<span className="text-xl text-gray-400 font-bold uppercase tracking-widest ml-1">{plan.price !== "Custom" ? "/mo" : ""}</span>
                                </div>
                                <p className="text-gray-600 dark:text-gray-400 font-medium leading-relaxed">{plan.desc}</p>
                            </div>

                            <ul className="space-y-6 mb-16 flex-grow">
                                {plan.features.map((feature, idx) => (
                                    <li key={idx} className="flex items-start gap-4 text-sm text-gray-900 dark:text-white font-bold group-hover:text-blue-600 transition-colors">
