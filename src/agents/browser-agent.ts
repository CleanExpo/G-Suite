...
export class BrowserAgent extends BaseAgent {
  ...
  readonly description =
    'Autonomous web browser agent for navigation, interaction, and data extraction';
    ...
    try {
      switch (operation.action) {
        ...
        case 'extract': {
          ...
          if (this.session && this.session.pageStates.length > 0) {
            this.session.pageStates[this.session.pageStates.length - 1].extractedData =
              extractedData;
          }
  ...
  private describeOperation(op: BrowserOperation): string {
    switch (op.action) {
      case 'navigate':
        return `Navigate to ${op.value}`;
      case 'click':
        return `Click on ${op.target?.value}`;
      case 'type':
        return `Type "${op.value}" into ${op.target?.value}`;
      case 'scroll':
        return `Scroll ${op.options?.direction || 'down'}`;
      case 'screenshot':
        return 'Capture screenshot';
      case 'extract':
        return `Extract data from ${op.target?.value || 'page'}`;
      case 'wait':
        return `Wait ${op.options?.duration || 1000}ms`;
      case 'assert':
        return `Verify ${op.target?.value} ${op.options?.condition}`;
      default:
        return `${op.action} operation`;
    }
  }
