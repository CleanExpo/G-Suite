...
export class CompetitiveIntelligenceAgent extends BaseAgent {
    ...
    readonly description = 'Monitors competitors, tracks market changes, and provides strategic intelligence';
    ...
    private readonly model = genAI.getGenerativeModel({
        ...
        systemInstruction: 'You are a competitive intelligence analyst with expertise in market research, competitor analysis, and strategic planning. Provide actionable insights based on data.'
    });
    ...
    async plan(context: AgentContext): Promise<AgentPlan> {
        ...
        const competitors = competitorMatch
            ? competitorMatch[1].split(',').map(c => c.trim())
            : [];
        ...
        return {
            ...
            reasoning: 'Multi-phase competitive analysis: discovery, data collection, analysis, trend detection, synthesis'
        };
    }
    ...
    private async synthesiseIntelligence(findings: Record<string, unknown>): Promise<MarketIntelligence> {
        try {
            const prompt = `
    ...
    async verify(result: AgentResult, context: AgentContext): Promise<VerificationReport> {
        ...
        return {
            ...
            recommendations: intelligence?.recommendations ?? ['Re-run analysis with more specific targets']
        };
    }
}
