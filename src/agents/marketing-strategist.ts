...
export class MarketingStrategistAgent extends BaseAgent {
  ...
  readonly description =
    'Campaign planning and growth strategy agent with Gemini 3 and video generation';
  ...
  private readonly model = genAI.getGenerativeModel({
    ...
    systemInstruction:
      'You are an elite marketing strategist with expertise in digital campaigns, growth hacking, and multi-channel marketing. Think like a CMO at a Fortune 500 company. You are highly culturally aware and an expert in localizing global brands to specific regions, languages, and cultural nuances.',
  });
  ...
  private createFallbackPlan(context: AgentContext): AgentPlan {
    return {
      steps: [
        {
          id: 'research',
          action: 'Deep market research',
          tool: 'deep_research',
          payload: { topic: context.mission, depth: 'deep' },
        },
        {
          id: 'strategy',
          action: 'Generate strategy copy',
          tool: 'gemini_3_flash',
          payload: { prompt: `Marketing strategy for: ${context.mission}` },
        },
        {
          id: 'visuals',
          action: 'Create campaign visuals',
          tool: 'image_generation',
          payload: { prompt: `Marketing visual for ${context.mission}` },
        },
        {
          id: 'video',
          action: 'Generate video ad',
          tool: 'veo_31_generate',
          payload: { prompt: `Video ad: ${context.mission}`, duration: 6 },
        },
        {
          id: 'launch',
          action: 'Social media blast',
          tool: 'social_blast',
          payload: { content: context.mission },
        },
      ],
      estimatedCost: 200,
  ...
  async execute(plan: AgentPlan, context: AgentContext): Promise<AgentResult> {
    this.mode = 'EXECUTION';
    this.log(
      '⚡ Executing marketing campaign with enhanced tools...',
