...
import { createMissionStatus, updateMissionStatus, addMissionLog } from '../tools/firebaseSkills';
...
export class MissionOverseerAgent extends BaseAgent {
  ...
  readonly description =
    'High-level orchestrator that supervises the entire mission lifecycle with independent verification';
  ...
  private async analyze(context: AgentContext): Promise<{
    ...
    if (
      context.config &&
      context.config.explicitAgents &&
      Array.isArray(context.config.explicitAgents)
    ) {
      this.log(
        `🎯 Using explicit agents from context: ${context.config.explicitAgents.join(', ')}`,
      );
      return {
    ...
    if (mission.includes('marketing')) {
      ...
      mission.includes('invoice') ||
      mission.includes('receipt') ||
      mission.includes('contract') ||
      mission.includes('document')
    ) {
    ...
    if (historicalData.success && historicalData.totalMissions > 0) {
      this.log(
        `📊 Historical: ${historicalData.totalMissions} missions, ${historicalData.successRate.toFixed(1)}% success`,
      );
    ...
    try {
      ...
      const pool = new ExecutionPool({
        ...
        onTaskFail: async (task: SchedulerTask) => {
          ...
          if (this.missionState?.firebaseMissionId) {
            await addMissionLog(this.missionState.firebaseMissionId, `Failed: ${task.step.action}`);
          }
        },
      });
      ...
      this.addLog(
        `📊 Quality Score: ${qualityScore.toFixed(1)}% (Threshold: ${this.qualityThreshold * 100}%)`,
      );

      if (
        qualityScore < this.qualityThreshold * 100 &&
        this.missionState.retryCount < this.maxRetries
      ) {
      ...
      result.data = {
        ...
        schedulerMetrics: {
          ...
          parallelEfficiency:
            schedulerResult.totalDurationMs > 0
  ...
  private async executeAgentStep(
    ...
    this.addLog(
      `${agentName} executed. Success: ${agentResult.success}, Cost: ${agentResult.cost}, Duration: ${agentResult.duration}ms`,
    );
  ...
  private calculateQualityScore(audits: VerificationReport[]): number {
    ...
    const passedChecks = audits.reduce(
      (sum, audit) => sum + audit.checks.filter((check) => check.passed).length,
      0,
    );
  ...
  private async handleAdaptiveRetry(
    ...
    const failurePattern = this.analyzeFailurePattern(this.missionState!.independentAudits);
  ...
  private analyzeFailurePattern(audits: VerificationReport[]): {
    ...
    // Categorize failures
    const hasFileMissing = failedChecks.some(
      (c) =>
        c.message?.toLowerCase().includes('not found') ||
        c.message?.toLowerCase().includes('missing'),
    );

    const hasTestFailure = failedChecks.some(
      (c) => c.name.toLowerCase().includes('test') || c.message?.toLowerCase().includes('test'),
    );

    const hasContentIssue = failedChecks.some(
      (c) =>
  ...
  private async dynamicReplan(
    ...
    const issues = intermediateResults.flatMap((r) => r.uncertainties ?? []).filter((u) => u);
  ...
  private async finalize(
    ...
    this.addLog(
      `📊 Cost Telemetry: ${totalCost} credits, ${totalTokens} tokens across ${Object.keys(agentCosts).length} agents`,
    );
  ...
  private async recordLearning(
    ...
    try {
      ...
      this.log(
        `📚 Learning recorded: ${missionType} (${learning.success ? 'success' : 'failure'})`,
      );
    } catch (error: any) {
  ...
  private async queryHistoricalPatterns(missionType: string): Promise<{
    ...
    try {
      ...
      const avgDuration =
        learnings.reduce((sum: number, l: any) => sum + l.duration, 0) / learnings.length;
      ...
      const avgRetries =
        learnings.reduce((sum: number, l: any) => sum + l.retryCount, 0) / learnings.length;
