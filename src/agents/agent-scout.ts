...
export class AgentScoutAgent extends BaseAgent {
    ...
    readonly description = 'Reconnaissance agent that discovers and evaluates agents from external sources';
    ...
    private determineMissionType(context: ScoutContext): ScoutMissionType {
        ...
        if (mission.includes('trend') || mission.includes('watch') || mission.includes('monitor')) return 'trend_watch';
    ...
    private async analyzeFleetGaps(): Promise<string[]> {
        ...
        for (const expected of expectedCapabilities) {
            const isCovered = Array.from(coveredCapabilities).some(cap =>
                cap.includes(expected) || expected.includes(cap)
            );
            if (!isCovered) {
                gaps.push(expected);
            }
    ...
    async plan(context: AgentContext): Promise<AgentPlan> {
        ...
        steps.push({
            ...
            payload: { missionType },
            dependencies: missionType === 'agent_audit'
                ? ['security_audit']
        ...
        return {
            ...
            reasoning: `${missionType} mission targeting ${sourcesToScan.length} sources. ` +
    ...
    async execute(plan: AgentPlan, context: AgentContext): Promise<AgentResult> {
        ...
        try {
            ...
            const compatibleAgents = dossiers.filter(d =>
                this.calculateCompatibilityScore(d) >= compatibilityThreshold
            );
            ...
            const scoutReport: ScoutReport = {
                ...
                flaggedForReview: dossiers.filter(d => d.securityRating === 'C' || d.securityRating === 'D').length,
