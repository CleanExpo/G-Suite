...
export class UIAuditorAgent extends BaseAgent {
  ...
  readonly description =
    'Visual QA agent that analyzes screenshots for UI issues and orchestrates fixes';
  ...
  async plan(context: AgentContext): Promise<AgentPlan> {
    ...
    return {
      ...
      reasoning:
        `UI Audit complete. Quality Score: ${qualityScore}%. ` +
  ...
  async verify(result: AgentResult, context: AgentContext): Promise<VerificationReport> {
    ...
    const checks = [
      ...
      },
      {
        ...
        passed:
          (this.currentAudit?.issues.filter((i) => i.severity === 'critical').length ?? 0) === 0 ||
          issuesFixed > 0,
