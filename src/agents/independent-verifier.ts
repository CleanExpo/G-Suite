...
export class IndependentVerifierAgent extends BaseAgent {
  ...
  async verify(result: AgentResult, context: AgentContext): Promise<VerificationReport> {
    ...
    if (!result.data || typeof result.data !== 'object') {
      return {
        passed: false,
        checks: [
          { name: 'Data Integrity', passed: false, message: 'No structured data to verify' },
        ],
      };
    }
        ...
        switch (criterion.type) {
          ...
          case 'content_contains':
            ...
            check.message = check.passed
              ? `Contains '${criterion.expected}'`
              : `Missing '${criterion.expected}'`;
            break;
  ...
  private async verifyVisualQuality(
    urlOrPath: string,
