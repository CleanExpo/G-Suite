...
export class SocialCommanderAgent extends BaseAgent {
  ...
  private readonly model = genAI.getGenerativeModel({
    ...
    systemInstruction:
      'You are a viral social media strategist. Create engaging, platform-optimized content that drives engagement.',
  });
  ...
  async plan(context: AgentContext): Promise<AgentPlan> {
    ...
    const targetPlatforms = this.platforms.filter(
      (p) => context.mission.toLowerCase().includes(p) || configPlatforms?.includes(p),
    );
    ...
    const steps: PlanStep[] = [
      // Deep research on trends first
      {
        ...
        tool: 'deep_research',
        payload: {
          topic: `trending ${platforms.join(' ')} topics for: ${context.mission}`,
          depth: 'moderate',
        },
      },
