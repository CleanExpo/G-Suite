...
export class BackendSpecialistAgent extends BaseAgent {
  ...
  readonly requiredSkills = ['nodejs', 'typescript', 'api_design', 'prisma', 'testing'];
...
- Australian English in documentation`,
  ...
  private analyseRequirements(context: AgentContext): BackendTask[] {
    ...
    if (
      mission.includes('integrate') ||
      mission.includes('external') ||
      mission.includes('third-party')
    ) {
      tasks.push({
            ...
            value: {
              checks: [
                { check: 'authentication', status: 'pass', details: 'JWT validation in place' },
                {
                  check: 'sql_injection',
                  status: 'pass',
                  details: 'Prisma ORM prevents injection',
                },
                {
                  check: 'rate_limiting',
                  status: 'pass',
                  details: 'Rate limiter middleware active',
                },
                {
                  check: 'input_validation',
                  status: 'pass',
                  details: 'Zod schemas validate input',
                },
              ],
