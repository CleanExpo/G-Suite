...
export class SEOAnalystAgent extends BaseAgent {
  ...
  readonly description =
    'Search optimization agent with Gemini 3 and comprehensive audit capabilities';
  ...
  private createComprehensivePlan(url: string, mission: string): AgentPlan {
    return {
      steps: [
        ...
        {
          id: 'gsc_data',
          action: 'Search Console analysis',
          tool: 'search_console_audit',
          payload: { siteUrl: url },
        },
        {
          id: 'competitors',
          action: 'Deep competitor research',
          tool: 'deep_research',
          payload: { topic: `SEO competitors for ${url}`, depth: 'deep' },
        },
        {
          id: 'keywords',
          action: 'Keyword opportunity analysis',
          tool: 'gemini_3_flash',
          payload: { prompt: `Identify top 20 keyword opportunities for: ${mission}` },
        },
        {
          id: 'content',
          action: 'Content gap analysis',
          tool: 'gemini_3_flash',
          payload: { prompt: `Analyze content gaps and opportunities for: ${url}` },
        },
      ],
      estimatedCost: 125,
  ...
  async verify(result: AgentResult, context: AgentContext): Promise<VerificationReport> {
    ...
    return {
        ...
        },
        {
          ...
          passed:
            result.artifacts?.some(
              (a) => a.name.includes('competitors') || a.name.includes('keywords'),
            ) ?? false,
      ...
      recommendations: result.success
        ? [
            'Implement priority fixes within 7 days',
